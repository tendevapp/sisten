...
| 4 | **Views monolíticas** (Compras 2.900, AdminPanel 1.776 linhas) com fetch, estado, regra e UI misturados | `src/views/*` | Reuso zero (cada tela reimplementa tabela, ordenação, colunas configuráveis, export XLSX, copy-to-clipboard) |
...
| **4. Central Compras + Dashboards SAP** | operação de suprimentos | Compras (cotação, edição obs/entrega/item_status com histórico), SapDashboards + DemandDashboard (vw_demandas, resolveComprador, resolveDataCorte) |
