...
* **Painel SAP (`/suprimentos/painel`) — `SapPanel.tsx`:** Painel tático de cruzamento das planilhas `ME5A` (Requisições de Compra) e `ZL0132` (Pedidos de Compra). Exibe indicadores de lead time por grupo comprador, farol de atrito/atraso, dias em aberto e permite ao comprador registrar previsões de entrega e observações operacionais.
* **Central de Compras (`/suprimentos/fornecedores-sem-po`) — `SuppliersNoPO.tsx`:** Tela para análise e tratamento de RIs em aberto sem Pedido de Compra (PO) gerado. Permite ao comprador agrupar itens por grupo de mercadoria, registrar histórico de cotação enviada e atribuir fornecedores cotados.
