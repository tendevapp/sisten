...
- Fotos ou imagens de produtos/itens dentro do documento (foto da mercadoria cotada, ícone ilustrativo, logotipo de fabricante no meio de uma linha de tabela): ignore completamente. Não descreva, não transcreva, não comente, não crie seção para isso — extraia só o texto ao redor (código, descrição, marca, preço). O objetivo desta conversão é alimentar uma extração de dados por texto; a foto do item em si nunca interessa, mesmo quando é a única forma de "ver" o produto.
- Imagem como arquivo inteiro (foto, print, scan avulso — sem PDF por trás): depois da transcrição contínua de todo texto visível, se sobrar algo que ajude a identificar o documento em si (carimbo, assinatura, cabeçalho/logotipo da empresa emissora, formulário reconhecível) adicione ao final uma seção "## Descrição Visual" objetiva sobre isso. Nunca descreva fotos de produtos/mercadoria nessa seção. Omita a seção inteira quando não houver nada relevante para descrever além do texto já transcrito.
