-- =====================================================================
-- Recebimento e contagem — check "parcial" por item.
--
-- O PO tem entregas parciais (mesma lógica da Central de Compras: coluna
-- `qtd_fornecida` da ZL0132). O pendente já é `qtd_pedido - qtd_fornecida`.
-- Quando o lote que chegou agora ainda é MENOS que esse pendente mas está
-- certo — o resto vem depois —, o conferente marca `parcial` e a linha NÃO
-- vira falta / NC. Receber a mais continua sendo excedente.
-- =====================================================================

alter table public.alm_receb_conferencia_itens
  add column if not exists parcial boolean not null default false;

-- registrar_conferencia: + parcial no insert dos itens
create or replace function public.alm_receb_registrar_conferencia(
  p_cab jsonb, p_itens jsonb, p_nc jsonb default null
)
...
begin
  ...
  insert into public.alm_receb_conferencia_itens
    (conferencia_id, linha_ref, nro_pedido, material_code, descricao, unidade, qtd_pedido, qtd_ja_fornecida,
     qtd_recebida, conferido, divergencia, tipo_divergencia, item_manual, parcial, observacao, evidencias)
  select
    ...
    coalesce((x->>'item_manual')::boolean, false), coalesce((x->>'parcial')::boolean, false),
    nullif(x->>'observacao',''), coalesce(x->'evidencias', '[]'::jsonb)
  from jsonb_array_elements(p_itens) x;
