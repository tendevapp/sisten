-- FBL1N stores several supplier invoice references with a trailing SAP suffix
-- such as "342590-1" or "940414-0", while ZL0136 stores the fiscal NF as
-- "342590" or "940414". The suffix is not part of the NF identity.
--
-- This preserves the supplier equality and only removes a final -N or /N.
-- The rule was validated against the current snapshot: all new matches were
-- unambiguous by supplier and normalized reference.
...
with pagamentos_fbl1n as (
  select
    ...
    nullif(regexp_replace(ltrim(btrim(f.referencia), '0'), '[-/][0-9]+$', ''), '') as numero_nf_normalizado,
  ...
  where f.montante_moeda_doc < 0
    ...
    and nullif(regexp_replace(ltrim(btrim(f.referencia), '0'), '[-/][0-9]+$', ''), '') is not null
  group by
    f.fornecedor,
    nullif(regexp_replace(ltrim(btrim(f.referencia), '0'), '[-/][0-9]+$', ''), '')
), faturas as (
  select
