-- =====================================================================
-- Financeiro > Faturamento GW Jacobina — edição liberada a quem acessa a
-- página, não só ao autor do lançamento ou a um admin.
--
-- Foge do padrão autor-ou-admin dos demais formulários (`form_pode_editar`,
-- ver 20260902160000) de propósito: o time de Faturamento é pequeno e
-- trabalha o mesmo lançamento em conjunto (quem cadastra a torre não é
-- sempre quem lança a NF). A página já é a barreira — só entra quem tem
-- acesso ao módulo Financeiro (`fin_faturamento_gwjaco`, ver `pages.ts`); a
-- RLS aqui só garante que quem chega lá dentro consegue editar qualquer
-- linha, não só a que criou.
--
-- Não mexe em `form_pode_editar`: é a função compartilhada por outros 13
-- formulários, e eles continuam autor-ou-admin.
-- =====================================================================

drop policy if exists fin_fat_gwjaco_upd on public.fin_fat_gwjaco;
create policy fin_fat_gwjaco_upd on public.fin_fat_gwjaco
  for update to authenticated using (true) with check (true);

drop policy if exists fin_fat_gwjaco_del on public.fin_fat_gwjaco;
create policy fin_fat_gwjaco_del on public.fin_fat_gwjaco
  for delete to authenticated using (true);

-- A RPC de edição não trava mais por autor — só valida que o registro existe.
