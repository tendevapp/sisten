-- =====================================================================
-- Recebimento — vários POs numa conferência + NCR editável com ações.
--
--   - `alm_receb_conferencia_itens.nro_pedido` — o PO de cada linha; uma
--     conferência agora pode misturar itens de pedidos diferentes.
--   - `alm_receb_conferencias.pedidos text[]` — todos os POs da conferência
--     (o `nro_pedido` continua sendo o "principal", 1º da lista).
--   - `alm_receb_nc.acoes jsonb` — linha do tempo de andamentos da tratativa
--     ({texto, evidencias, por_nome, em}).
--   - `alm_receb_alteracoes.entidade` passa a aceitar 'nc'.
--   - RPC `alm_receb_editar_nc` — status/severidade/responsável/descrição +
--     registrar ação (texto + foto) + foto geral, com log. Sem trava de
--     autor: a tratativa da NCR é de quem trata.
-- =====================================================================

alter table public.alm_receb_conferencia_itens add column if not exists nro_pedido text;
alter table public.alm_receb_conferencias add column if not exists pedidos text[] not null default '{}';
alter table public.alm_receb_nc add column if not exists acoes jsonb not null default '[]'::jsonb;

alter table public.alm_receb_alteracoes drop constraint if exists alm_receb_alteracoes_entidade_check;
alter table public.alm_receb_alteracoes add constraint alm_receb_alteracoes_entidade_check
  check (entidade in ('carga','conferencia','nc'));

-- --- registrar_conferencia: + nro_pedido por item, + pedidos[] no cabeçalho
...
declare
  ...
  v_pedidos text[] := coalesce(array(select distinct e from jsonb_array_elements_text(coalesce(p_cab->'pedidos','[]'::jsonb)) e where e <> ''), '{}');
begin
...
-- --- editar_conferencia: idem (nro_pedido por item + pedidos[])
...
begin
  ...
  v_pedidos := case when p_cab ? 'pedidos'
    then coalesce(array(select distinct e from jsonb_array_elements_text(p_cab->'pedidos') e where e <> ''), '{}')
...
-- --- editar_nc: tratativa da NCR (sem trava de autor)
create or replace function public.alm_receb_editar_nc(
  p_id uuid, p_patch jsonb default '{}'::jsonb, p_acao jsonb default null, p_user jsonb default '{}'::jsonb
)
returns jsonb
language plpgsql
security definer
...
declare
  ...
  v_alt jsonb := '[]'::jsonb;
  v_campos text[] := array['status','severidade','responsavel','descricao','tipo','resolucao'];
  k text;
  v_tem_acao boolean := p_acao is not null and p_acao <> 'null'::jsonb
    and (coalesce(nullif(p_acao->>'texto',''), '') <> '' or coalesce(jsonb_array_length(p_acao->'evidencias'),0) > 0);
begin
  select * into v_old from public.alm_receb_nc where id = p_id and not excluido;
  if not found then raise exception 'Nao conformidade nao encontrada.'; end if;

  foreach k in array v_campos loop
    if p_patch ? k then
      if coalesce((to_jsonb(v_old) ->> k),'') is distinct from coalesce((p_patch ->> k),'') then
        v_alt := v_alt || jsonb_build_object('campo', k, 'de', to_jsonb(v_old) ->> k, 'para', p_patch ->> k);
      end if;
    end if;
  end loop;
  if v_tem_acao then
    v_alt := v_alt || jsonb_build_object('campo','acao','de','','para', coalesce(nullif(p_acao->>'texto',''), '(foto)'));
  end if;

  update public.alm_receb_nc set
    status = coalesce(nullif(p_patch->>'status',''), status),
    severidade = coalesce(nullif(p_patch->>'severidade',''), severidade),
    responsavel = case when p_patch ? 'responsavel' then nullif(p_patch->>'responsavel','') else responsavel end,
    descricao = coalesce(nullif(p_patch->>'descricao',''), descricao),
    tipo = coalesce(nullif(p_patch->>'tipo',''), tipo),
    resolucao = case when p_patch ? 'resolucao' then nullif(p_patch->>'resolucao','') else resolucao end,
    evidencias = coalesce(p_patch->'evidencias', evidencias),
    acoes = case when v_tem_acao then acoes || jsonb_build_object(
                   'texto', nullif(p_acao->>'texto',''),
                   'evidencias', coalesce(p_acao->'evidencias', '[]'::jsonb),
                   'por_id', nullif(p_user->>'id',''),
                   'por_nome', p_user->>'nome',
                   'em', now())
                 else acoes end,
    resolvida_em = case when coalesce(nullif(p_patch->>'status',''), status) = 'resolvida'
                          and v_old.status <> 'resolvida' then now()
                        when coalesce(nullif(p_patch->>'status',''), status) <> 'resolvida' then null
                        else resolvida_em end
  where id = p_id;

  if jsonb_array_length(v_alt) > 0 then
    insert into public.alm_receb_alteracoes
      (entidade, entidade_id, codigo, alteracoes, resumo, alterado_por_id, alterado_por_nome)
    values ('nc', p_id, v_old.codigo, v_alt,
            jsonb_array_length(v_alt) || ' alteracao(oes)',
            nullif(p_user->>'id','')::uuid, p_user->>'nome');
  end if;
...
revoke all on function public.alm_receb_editar_nc(uuid, jsonb, jsonb, jsonb) from public, anon;
grant execute on function public.alm_receb_editar_nc(uuid, jsonb, jsonb, jsonb) to authenticated, service_role;
