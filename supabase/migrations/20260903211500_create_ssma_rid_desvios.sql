...
-- Bucket para armazenamento de fotos do RID
insert into storage.buckets (id, name, public)
values ('ssma-desvios', 'ssma-desvios', false)
on conflict (id) do nothing;

-- Políticas de Storage para o bucket ssma-desvios (alinhado a expedicao-fotos)
drop policy if exists "ssma_desvios_objects_insert" on storage.objects;
create policy "ssma_desvios_objects_insert" on storage.objects
  for insert to authenticated
  with check (bucket_id = 'ssma-desvios');

drop policy if exists "ssma_desvios_objects_read" on storage.objects;
create policy "ssma_desvios_objects_read" on storage.objects
  for select to authenticated
  using (bucket_id = 'ssma-desvios');

drop policy if exists "ssma_desvios_objects_update" on storage.objects;
create policy "ssma_desvios_objects_update" on storage.objects
  for update to authenticated
  using (bucket_id = 'ssma-desvios');

drop policy if exists "ssma_desvios_objects_delete" on storage.objects;
create policy "ssma_desvios_objects_delete" on storage.objects
