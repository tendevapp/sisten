-- =====================================================================
-- Requisição no Balcão: múltiplos grupos de PEP e liberação de itens sem saldo.
--
-- 1) `alm_req_balcao_itens`:
--    - `aplicacao_pep` e `aplicacao` por item, permitindo que uma mesma
--      requisição contenha itens de centros de custo/PEPs diferentes.
--    - `sem_saldo`: flag que sinaliza itens cujo saldo na ZL0024 era 0 ou
--      menor que a quantidade retirada (ex.: defasagem de importação).
--
-- 2) `alm_req_balcao_salvar`:
--    - Aceita PEP por item e não rejeita mais materiais sem saldo positivo
--      na ZL0024 (apenas grava o saldo encontrado, sinaliza sem_saldo e permite
--      o registro para conferência/confirmação antes da importação no SAP).
-- =====================================================================

alter table public.alm_req_balcao_itens
  add column if not exists aplicacao_pep text,
  add column if not exists aplicacao text,
  add column if not exists sem_saldo boolean not null default false;

create index if not exists idx_alm_req_balcao_itens_pep
  on public.alm_req_balcao_itens (aplicacao_pep);
...
begin
  ...
  -- Se v_pep estiver vazio no cabeçalho, tenta obter do primeiro item
  ...
  for v_item in select * from jsonb_array_elements(p_itens) loop
    ...
    insert into public.alm_req_balcao_itens
      (requisicao_id, ordem, material, descricao, unidade, quantidade, saldo_zl0024, aplicacao_pep, aplicacao, sem_saldo)
    values
      (v_row.id, v_ordem, v_mat, v_desc, coalesce(v_umb, 'UN'), v_qtd, v_saldo, v_item_pep, v_item_pep_nome, v_sem_saldo);
