-- Migration: Desconsiderar pedidos eliminados (eflag_e = 'L' / codigo_eliminacao = 'L') no fallback ME2L
-- Data: 2026-09-22
--
-- Contexto do Bug:
-- Quando um item do pedido de compras e eliminado/cancelado no SAP (marcado com icone de lixeira / LOEKZ = 'L'):
-- 1. A tabela sap_zl0132_po (mv_pedidos_por_ri) grava eflag_e = 'L'.
-- 2. Na view vw_sap_requisicoes_enriquecidas, p.eflag_e <> 'L' faz p.doc_compra ficar NULL (correto).
-- 3. Porem, a condicao p.doc_compra IS NULL ativava o fallback da me2l_por_reqc.
--    - A CTE me2l_por_reqc nao filtrava codigo_eliminacao <> 'L', pegando registros cancelados.
--    - Alem disso, quando o ME2L ainda nao sincronizou o codigo_eliminacao ou tinha historico antigo,
--      ele reatribuia o mesmo pedido que a ZL0132 ja sabia estar eliminado (eflag_e = 'L').
-- 4. Com isso, os itens cancelados reapareciam como "Processado" / origem_po = 'me2l' e
--    poluiam o Diligenciamento / Sem MIGO como se estivessem pendentes de entrega.
--
-- Solucao:
-- 1. Na CTE me2l_por_reqc: adicionar filtro AND COALESCE(upper(btrim(codigo_eliminacao)), '') <> 'L'.
-- 2. No JOIN com me2l_por_reqc: garantir que nao exista registro eliminado na mv_pedidos_por_ri
--    para aquele mesmo RI e documento_compras (AND NOT EXISTS pe.eflag_e = 'L').
...
WITH me2l_por_reqc AS (
  ...
  WHERE requisicao_compra IS NOT NULL 
    AND requisicao_compra <> ''
    AND COALESCE(upper(btrim(codigo_eliminacao)), '') <> 'L'
     ...
     SELECT r.ri,
       ...
       FROM sap_me5a_rc r
         ...
         LEFT JOIN me2l_por_reqc m2 ON m2.requisicao_compra = r.requisicao_de_compra 
                                   ...
                                   AND NOT EXISTS (
                                     SELECT 1 FROM mv_pedidos_por_ri pe 
                                     WHERE pe.ri = r.ri 
                                       AND pe.doc_compra = m2.documento_compras
                                       AND COALESCE(upper(btrim(pe.eflag_e)), '') = 'L'
                                   )
    ), c AS (
     SELECT j.ri,
        j.tipo_de_documento,
