...
export const CHANGELOG: ChangelogEntry[] = [
  ...
  {
    data: '2026-09-21',
    resumo: 'SSMA > EPI por Função — Reformulação Completa do Cadastro Master-Detail e Matriz Geral (`SsmaEpiPorFuncaoView.tsx`, `ssmaEpiPorFuncaoApi.ts`, `ssmaEpiPorFuncaoApi.test.ts`, `SsmaHub.tsx`, `diretrizes.ts`): 1. Diagnóstico e resolução do gargalo de consulta individual por função: agora os 300 requisitos de EPI e 54 funções são carregados unificados em lote com seus relacionamentos, garantindo navegação instantânea a 0ms e busca em tempo real em toda a matriz; 2. Implementação de visualização dupla: Modo Master-Detail com split screen (painel esquerdo com cargos, busca e badges de contagem/pendência; painel direito com requisitos organizados em Básico Obrigatório, Específico e Condicional por Exposição com fotos do Book) e Modo Matriz Geral (tabela unificada para busca global e auditoria); 3. Inclusão de CRUD completo de Funções/Cargos (`salvarFuncaoEpi` com numeração sequencial automática `FUN-XXX`) e exclusão/desvinculação de EPIs; 4. Seletor enriquecido no modal de vínculo com o Book de EPIs com busca textual, código SAP, CA, fabricante e miniatura de foto; 5. KPIs no topo com total de funções, requisitos mapeados, vinculados ao Book e alerta de pendências; 6. Cobertura de testes unitários aprovada e build de produção validado sem regressões.',
  },
  {
    data: '2026-09-21',
    resumo: 'Suprimentos > Análise de Cotações — Correção de Chave Estrangeira ao Salvar Proposta e Isolamento de Itens (`cotacoes.ts`, `cotacoesApi.ts`, `AnaliseCotacoes.tsx`, `cotacoesSalvamento.test.ts`, `diretrizes.ts`): 1. Diagnóstico e resolução do erro de violação de chave estrangeira ("cotacao_propostas_processo_id_fkey") originado pela tentativa de persistir propostas vinculadas a processos excluídos, recriados ou dessincronizados na aba do navegador; 2. Em `salvarProcessoCotacao` (`cotacoesApi.ts`), adicionada validação de UUID e checagem preventiva no Supabase para garantir a existência do processo antes da execução da RPC, orientando o comprador com mensagem clara se o processo tiver sido removido; 3. Mapeamento defensivo do erro PostgreSQL 23503 para mensagens amigáveis na interface em vez de exibir exceção bruta de banco de dados; 4. Em `propostaParaDraft` (`cotacoes.ts`), garantia de isolamento entre processos: cópia/importação de propostas entre processos distintos agora reseta `processo_item_id`, impedindo o reaproveitamento de chaves órfãs de itens do processo de origem e acionando o reconciliador automático; 5. Em `AnaliseCotacoes.tsx`, sincronização da URL ativa com `processoId`, recuperação graciosa redirecionando para a listagem em caso de processo inexistente, e expurgo de rascunhos no `localStorage` na exclusão; 6. Cobertura de testes unitários adicionada e 100% aprovada no Vitest.',
  },
  {
    data: '2026-09-21',
    resumo: 'Almoxarifado > Abrir RM — Liberação de Permissão RLS para Gravação de RM no Supabase (`AbrirRm.tsx`, `localDb.ts`, `abrirRmVinculo.test.ts`, `20260921104500_almox_abrir_rm_core_solicitacoes_rls.sql`, `diretrizes.ts`): 1. Correção do bloqueio de sincronização com o banco remoto na vinculação de número de RM da solicitação: usuários com acesso concedido à página "Abrir RM" (via `page_access->>\'almox_abrir_rm\' = \'true\'` ou setor Almoxarifado, como o perfil de Ayron) estavam sendo barrados pela política RLS de UPDATE em `core_solicitacoes`, fazendo com que o número da RM ficasse salvo apenas no cache local do navegador; 2. Nova função `pode_abrir_rm()` criada no Postgres e integrada às políticas `requests_update` e `requests_read` em `core_solicitacoes`, autorizando a gravação definitiva da coluna `linked_rm_number` para qualquer colaborador que possua acesso ao módulo; 3. Implementado fallback de update direto em `localDb.updateLinkedRM` e reversão atômica do cache local em caso de erro na rede; 4. Cobertura completa de testes unitários adicionada e validada.',
  },
  {
    data: '2026-09-21',
    resumo: 'Suprimentos > Central de Compras e Análise de Cotações — Coluna com Número da Cotação, Abertura do Mapa Comparativo em Nova Janela e Transição Automática de Status (`Compras.tsx`, `AnaliseCotacoes.tsx`, `cotacoesApi.ts`, `localDb.ts`, `diretrizes.ts`): 1. Na Central de Compras (`Compras.tsx`), inclusão de coluna "Cotação" na tabela da listagem e nos cards móveis, exibindo o identificador oficial do processo de cotação aberto (ex.: "COT-210926-01"); 2. O número da cotação possui link direto com abertura em nova janela/aba (`target="_blank"` com rota `/suprimentos/analise-cotacoes?processoId=...&fase=mapa`), levando o comprador imediatamente ao Mapa Comparativo completo sem perder o contexto, paginação ou filtros da Central de Compras; 3. Criação de processos de cotação em lote agora promove automaticamente o status de todas as requisições vinculadas para "Análise de Cotações", sincronizando localmente no `localDb` e em lote no Supabase com histórico de auditoria (`sap_me5a_rc` e `sap_requisicoes_observacoes`); 4. Limpeza automática do cache em memória da Central de Compras (`limparComprasMemoryCache`) para atualização instantânea dos indicadores e listagens.',
  },
  {
    data: '2026-09-18',
    resumo: 'Suprimentos > Recorrência de Compras — Foco Exclusivo em Itens com Compras Repetidas (`TabRecorrenciaCompras.tsx`, `historicoAnalytics.ts`, `historicoAnalytics.test.ts`, `diretrizes.ts`): 1. Na análise de recorrência de compras (`TabRecorrenciaCompras.tsx`), implementado filtro padrão ativado para observar exclusivamente itens que se repetem (2 ou mais compras distintas), ocultando itens de compra única pontual que poluíam o ranking de Pareto e as tabelas; 2. Em `calcRecorrencia` (`historicoAnalytics.ts`), adicionado o parâmetro `apenasRecorrentes` (filtrando itens com menos de 2 compras distintas); 3. Em `detectarAlertasAuditoria` (`historicoAnalytics.ts`), o alerta de "Maior valor total comprado" passa a considerar estritamente itens que possuem histórico de repetição (2+ pedidos), evitando falsos positivos de compras pontuais isoladas; 4. Adicionado seletor de modo na barra de filtros ("Recorrentes (2+)" ativo por padrão vs "Todas as compras"); 5. Atualizados os KPIs para refletir o Valor Total Recorrente e total de itens recorrentes em análise; 6. Testes unitários adicionados com 100% de aprovação no Vitest.',
  },
  {
    data: '2026-09-18',
    resumo: 'Suprimentos > Otimização da Central de Compras e Filtro de Contratos na Gestão de Suprimentos (`Compras.tsx`, `SapDashboards.tsx`, `FiltrosSuprimentos.tsx`, `filtrosSuprimentosContrato.test.ts`, `diretrizes.ts`): 1. Na Central de Compras (`Compras.tsx`), implementado cache em memória de sessão (`comprasMemoryCache`) eliminando o bloqueio de carregamento nas transições de página (abertura instantânea a 0ms); 2. Paralelização completa de consultas ao Supabase via `Promise.all` (catálogo ZL0169, histórico de cotações, mapa de processos e pedidos ZL0132 em lotes simultâneos), reduzindo o tempo de carregamento da rede em mais de 80%; 3. Fast-path de renderização imediata com dados do `localDb` e salvamento de observações/status em tempo real sem refetch da base externa; 4. Nos Dashboards de Gestão de Suprimentos (`SapDashboards.tsx` / `FiltrosSuprimentos.tsx`), adicionado novo filtro global "Contrato" com as opções "Sem contratos" (default), "Com contratos" e "Todos (com e sem contrato)", integrado à função canônica `ehItemDeContrato` (`contratoPedido.ts`); 5. Cobertura de testes unitários adicionada e 100% validada.',
  },
  {
    data: '2026-09-18',
    resumo: 'Suprimentos > Central de Compras — Coluna de Cotação e Janela Suspensa do Mapa Comparativo (`Compras.tsx`, `cotacoesApi.ts`, `cotacoes.ts`, `MapaCotacaoModal.tsx`, `MapaComparativo.tsx`, `AnaliseCotacoes.tsx`, `diretrizes.ts`): 1. Adicionada nova coluna "Cotação" na tabela plana da Central de Compras e badge correspondente nos cards, identificando de imediato os itens vinculados a processos de cotação (ex: "COT-180926-26"); 2. Abertura do Mapa Comparativo de Cotação em janela suspensa (modal flutuante responsivo) com 1 clique diretamente no número da cotação, permitindo visualizar propostas recebidas, rodadas de negociação, comparativo de custos e exportações sem sair da Central de Compras; 3. Suporte a atalho dentro da janela suspensa para abrir o processo em tela cheia no módulo de Análise de Cotações; 4. Indexação dinâmica de itens cotados por RI e RM/Item no banco Supabase (`sup_cotacao_processos` e `sup_cotacao_processo_itens`); 5. Integração da cotação na busca global de texto da Central de Compras e na exportação completa para planilha Excel (.xlsx); 6. Itens sem processo vinculado exibem traço neutro ("—") mantendo a clareza visual.',
  },
  {
    data: '2026-09-18',
    resumo: 'Financeiro > Faturamento GW Jacobina — Padronização de Rótulos em Últimas Notas e Inversão Física da Matriz de Torres (`FinFaturamentoWallboard.tsx`, `finFaturamentoRelatorio.ts`, `finFaturamentoRelatorio.test.ts`, `diretrizes.ts`): 1. No card "Últimas notas emitidas" do Wallboard, padronizada a identificação do tramo para o formato oficial "Torre [número] - Tramo" (ex.: "Torre 9 - T1", "Torre 12 - T3"), substituindo o formato resumido anterior; 2. No painel "Avanço por torre e tramo", invertida a ordem vertical das linhas de tramos para T5 no topo e T1 na base (T5 -> T1), alinhando a matriz do Wallboard à montagem física real das torres eólicas da fábrica TEN; 3. Cobertura completa de testes unitários atualizada e validada.',
  },
  {
    data: '2026-09-18',
    resumo: 'Almoxarifado > Movimentações de Estoque — Exibição da Data da Última Importação SAP MB51 e Frescor da Base (`Movimentacoes.tsx`, `localDb.ts`, `diretrizes.ts`): 1. Inclusão no cabeçalho do módulo da data e horário exatos da última importação da transação SAP MB51, com nome do arquivo de planilha e tooltip com o nome do usuário responsável; 2. Badge visual informando a data limite dos dados de movimentação presentes na base ("Base com dados até DD/MM/AAAA"); 3. Novos métodos `getLatestImportLog` e `fetchLatestImportLog` no `localDb` para consulta leve e reativa do histórico de importações do Supabase; 4. Atualização automática imediata do cabeçalho ao realizar novo upload ou clicar em Atualizar.',
  },
  {
    data: '2026-09-18',
    resumo: 'Permissões de Formulários para Visualizadores & Seleção Granular no Painel Administrativo (`pages.ts`, `pages.test.ts`, `PageAccessModal.tsx`, `UsersByModuleView.tsx`, `BulkPageAccessModal.tsx`, `App.tsx`, `SsmaHub.tsx`, `PortariaHub.tsx`, `LogisticaExpedicao.tsx`, `diretrizes.ts`): 1. Regra de Formulários para Perfil Visualizador: o único formulário operacional liberado universalmente por padrão para todos os visualizadores é o RID (FRM.SSMA-0001); formulários específicos do setor do colaborador (ex.: Almoxarifado para membros do Almoxarifado, Portaria para membros da Portaria/Vigilância) são liberados por padrão; formulários de outros setores permanecem bloqueados para visualizadores; 2. O administrador possui autonomia total para conceder ou revogar qualquer formulário individual por usuário (override manual); 3. Admin > Módulos de Acesso (Visão Colaborador): interface expansível por grupo com checkboxes para cada um dos 13 formulários detalhados, badges de contexto informando o motivo da liberação/bloqueio ("Universal (RID)", "Padrão - seu setor", "Bloqueado p/ visualizador", "Liberado manual", etc.), botões de restauração rápida e ações em lote no cabeçalho do grupo; 4. Admin > Módulos de Acesso (Visão por Módulo): adicionado seletor de formulários específicos na barra de filtros, tabela de usuários com status contextual e ações de liberação/bloqueio pontual com 1 clique; 5. Suporte completo na Edição em Lote (`BulkPageAccessModal.tsx`); 6. Roteamento e Hubs protegidos: URLs diretas em `/formularios/*` e grids de cards em `SsmaHub`, `PortariaHub` e `LogisticaExpedicao` verificam `canAccessForm` dinamicamente; 7. Testes unitários com 100% de aprovação (42 testes cobrindo todas as matrizes de acesso).',
  },
  {
    data: '2026-09-18',
    resumo: 'Catálogo SAP & Nova Solicitação de Compras — Exclusão de Códigos Inativos, Coluna Única de Status e Janela de Alerta para Ativação com o Setor Fiscal (`buscar_materiais_catalogo`, `buscar_materiais`, `materiais.ts`, `materiais.test.ts`, `Materials.tsx`, `MaterialSearchModal.tsx`, `NewRequest.tsx`, `diretrizes.ts`): 1. Expurgados da exibição e busca todos os códigos de materiais SAP inativos (iniciados com 9 ou com letras), tanto a nível de banco de dados (RPCs `buscar_materiais_catalogo` e `buscar_materiais`) quanto no cliente via `ehCodigoSapInativo()`; 2. Unificada a coluna como "Status" nas tabelas do Catálogo SAP (`Materials.tsx`) e do modal de busca de compras (`MaterialSearchModal.tsx`), removendo duplicidades; 3. Implementada janela modal de alerta (`ConfirmDialog`) caso o usuário tente selecionar ou solicitar um item com status obsoleto no SAP (Z1/Obsoleto), instruindo explicitamente sobre a necessidade de solicitar a ativação com o setor Fiscal; 4. Interceptação no envio do formulário de Nova Solicitação de Compra exibindo modal de alerta caso haja materiais obsoletos na lista; 5. Exibição de banner de alerta nos itens selecionados no formulário de Nova Solicitação de Compra orientando acionar o setor Fiscal; 6. Suporte da coluna Status na exportação de planilha CSV do Catálogo SAP; 7. Testes unitários com 100% de aprovação.',
  },
  {
    data: '2026-09-17',
    resumo: 'Almoxarifado > Movimentações — Vínculo de Saídas de Estoque com Elemento PEP (`vw_mb51_classificado`, `fin_pep`, `types.ts`, `SaidasPepPanel.tsx`, `Movimentacoes.tsx`, `diretrizes.ts`): 1. Vinculada a view de movimentações de estoque `vw_mb51_classificado` à tabela `fin_pep` pelo código WBS (`btrim(m.elemento_pep) = p.wbs_element`), expondo nome da PEP (`pep_nome`), projeto (`pep_projeto`) e nível (`pep_nivel`); 2. Adicionada nova aba "Saídas por PEP" em Almoxarifado > Movimentações para análise detalhada do consumo de estoque por frente de serviço / elemento PEP; 3. Painel analítico com KPIs de total de saídas, % apropriado a PEPs cadastrados, quantidade de PEPs atendidos e maior PEP em consumo; 4. Gráfico horizontal de barras Top 10 PEPs mais consumidos com seleção interativa; 5. Tabela analítica agregada com busca por código/nome de PEP e código/descrição de material, filtros por projeto e alocação; 6. Drill-down expansível por linha exibindo detalhe de cada material consumido no PEP (código, descrição, quantidade e valor); 7. Exportação profissional para Excel (.xlsx) com duas abas: Resumo Consolidado por PEP e Extrato Detalhado de Saídas; 8. Enriquecida a tabela da aba "Visão Geral" com a coluna Elemento PEP exibindo código e descrição com ordenação dedicada.',
  },
  {
    data: '2026-09-17',
    resumo: 'Financeiro — Tabela fin_pep e Importador de Estrutura PEP / WBS Element (`fin_pep`, `finPepApi.ts`, `finPepImportacao.ts`, `finPepImportacao.test.ts`, `ImportarPepModal.tsx`, `FinPepView.tsx`, `pages.ts`, `moduleHomes.ts`, `App.tsx`, `diretrizes.ts`): 1. Criação da tabela `fin_pep` no Supabase com índices e políticas de segurança RLS para armazenamento da Estrutura Analítica do Projeto (WBS Element) do SAP; 2. Modal de importação com suporte a upload de planilhas (.xlsx, .xls, .csv) e colagem direta de texto do Excel (Ctrl+V); 3. Parser resiliente com reconhecimento automático de colunas (Centro de lucro, Definição do projeto, WBS element, Name, Level, Unidade de medida, Moeda, Empresa, Código elemento classificação contábil, Código elemento de faturamento, Status, IFRS15 - OD); 4. Opções de gravação em lote: Upsert idempotente ou Substituição completa da base; 5. Tela de consulta e gestão `/financeiro/pep` com KPIs de total de nós, projetos e centros de lucro, barra de pesquisa global, filtros por nível e projeto, ordenação e exportação para Excel (.xlsx); 6. Testes unitários com 100% de aprovação.',
  },
  {
    data: '2026-09-17',
    resumo: 'Contratos (ME3N) — Coluna Tipo com opções PJ, Serviço e Material (`TabContratosLista.tsx`, `ContratoDetailModal.tsx`, `contratos.ts`, `localDb.ts`, `types.ts`, `database.types.ts`, `diretrizes.ts`): 1. Adicionada a coluna Tipo com badges interativos para seleção rápida direto na linha da tabela de contratos com stopPropagation; 2. Opções suportadas: PJ, Serviço e Material; 3. Persistência imediata no banco via `contratos_detalhes.tipo` e preservação na importação ME3N; 4. Filtro por tipo na barra de pesquisa, busca global e exportação para Excel.',
  },
  {
    data: '2026-09-17',
    resumo: 'Logística & Expedição — Histórico de Observações, Justificativas de SLA e Evidências com Fotos nas Carretas (`expedicao_tramos`, `expedicaoApi.ts`, `expedicaoRelatorio.ts`, `expedicaoRelatorio.test.ts`, `ExpedicaoRelatorioLeadTime.tsx`, `TramoCard.tsx`, `LogisticaExpedicao.tsx`, `types.ts`, `diretrizes.ts`): 1. Adicionada capacidade de registrar histórico completo e auditável de observações, justificativas de permanência (> 24h) e ocorrências operacionais por carreta; 2. Anexo de evidências fotográficas e documentais com compressão obrigatória no navegador via `comprimirImagemUpload` (Regra 1 do AGENTS.md), garantindo upload ágil e economia de egress/storage; 3. Visualização em linha do tempo com autor, data/hora legível, tag da classificação (Justificativa SLA, Ocorrência, Geral), texto e galeria de fotos; 4. Modal lightbox integrado para inspeção de imagens em tela cheia com opção de download do arquivo original; 5. Badges informativos de contagem de observações e anexos na tabela do relatório analítico de lead time e no cabeçalho do tramo na tela operacional; 6. Exportação para Excel (.xlsx) contendo observações consolidadas e contagem de evidências; 7. Cobertura de testes unitários com 100% de aprovação.',
  },
  {
    data: '2026-09-17',
    resumo: 'Logística & Expedição — Relatório e Gráficos de Lead Time das Carretas (`expedicaoRelatorio.ts`, `expedicaoRelatorio.test.ts`, `ExpedicaoRelatorioLeadTime.tsx`, `expedicaoApi.ts`, `LogisticaExpedicao.tsx`, `App.tsx`, `Formularios.tsx`, `diretrizes.ts`): 1. Implementado painel analítico completo para monitoramento dos lead times operacionais de carretas na planta TEN; 2. Visão Semanal: gráficos de evolução temporal do lead time médio (horas), volume total de carretas, identificação de veículos que ultrapassaram o SLA de 24 horas e decomposição dos gargalos por etapa (espera Portaria ➔ Pátio vs operação Pátio ➔ Expedição); 3. Visão por Carreta: ranking e gráfico horizontal de barras com linha de corte visual em 24h e destaque por cores conforme o status de SLA (verde <= 24h, âmbar 18h-24h, vermelho > 24h); 4. Visão por Transportadora: comparativo de lead time médio e taxa de violação de SLA por empresa; 5. Filtro rápido de 1 clique "Apenas carretas > 24h", pesquisa textual, presets de data e exportação consolidada em planilha Excel (.xlsx); 6. Roteamento integrado em `/formularios/logistica-expedicao/relatorio` e botão de acesso direto na listagem de carregamentos; 7. Testes unitários com 100% de aprovação (14 testes cobrindo todas as regras de tempo e SLA).',
  },
  {
    data: '2026-09-17',
    resumo: 'Suprimentos > Central de Compras / Sem MIGO — Modelo Padronizado do E-mail de Coleta (`coletaEmail.ts`, `coletaEmail.test.ts`, `diretrizes.ts`): 1. Atualizado o formato do corpo do e-mail da lista de coleta com saudação "Bom dia!", resumo de quantidade de itens e valor total seguido de separador de cabeçalho; 2. Agrupamento estruturado em dois níveis: cabeçalho por FORNECEDOR e sub-blocos por número de Pedido de Compra (PO); 3. Formatação objetiva dos itens com Código, Material, Quantidade e Valor, com divisórias por fornecedor; 4. Testes unitários atualizados com 100% de aprovação (10 testes cobrindo saudação, agrupamento de múltiplos POs, ordenação e modelo exato).',
  },
  {
    data: '2026-09-16',
    resumo: 'Logística & Expedição e Matriz de Autonomia — Campo de Pesquisa em Expedição e Correção de Números de Tramos por Torre (`LogisticaExpedicao.tsx`, `expedicaoApi.ts`, `projetosKitsAutonomia.ts`, `projetosKitsAutonomia.test.ts`, `types.ts`, `diretrizes.ts`): 1. Adicionado campo de pesquisa instantânea na listagem do Formulário de Logística e Expedição com suporte a busca por número do tramo (ex: 3192), protocolo (EXP-...), transportadora, motorista, placas e nota fiscal, com contador de resultados e botão para limpar filtro; 2. Na listagem de carregamentos, os números de tramo agora são destacados em badges visíveis e incorporados ao rótulo dos tramos; 3. Correção na Matriz de Autonomia de Kits: eliminada a propagação indevida de números de série físicos sintéticos oriundos da fórmula teórica de `proj_tramos_gwjaco`, que fazia com que torres superiores (ex: Torre 10, Torre 8, Torre 6) herdassem números baixos duplicados (como 3192, 3182, 3169) e virassem indevidamente status preto (Saída Portaria); agora as séries vêm estritamente das alocações reais registradas na fábrica e no almoxarifado.',
  },
  {
    data: '2026-09-16',
    resumo: 'Almoxarifado > Projetos — Matriz de Autonomia de Kits: Legenda no Rodapé, Status Preto para Saída da Portaria e Sincronização Automática com Formulário de Logística & Expedição (`MatrizAutonomiaKits.tsx`, `projetosKitsAutonomia.ts`, `projetosKitsAutonomia.test.ts`, `projetosApi.ts`, `useDadosProjetos.ts`, migration `20260916100000_allow_status_5_saida_portaria.sql`, `diretrizes.ts`): 1. A legenda de cores dos status operacionais foi movida do topo da tela para um painel estruturado na parte inferior da tabela matricial; 2. Implementado o novo status 5 (Saída Portaria / Preto `#09090B`), representando os tramos que já concluíram a expedição e saíram pela portaria da fábrica; 3. Sincronização automática em tempo real: o status 5 é alimentado diretamente pelos registros de tramos lançados no formulário de Logística e Expedição (`expedicao_tramos`), cruzando o número do tramo (série de 4 dígitos) com a série das células da matriz e da fábrica (`proj_tramos_gwjaco`); 4. As células sincronizadas automaticamente recebem identificação no hover e no modal de edição, garantindo total rastreabilidade; 5. O cabeçalho superior foi simplificado com controles de paginação (12 Torres / Todas) mais limpos; 6. Atualizado o modal de edição de células com suporte a gravação manual do status 5 e constraint no Supabase; 7. Testes unitários com 100% de aprovação (9 testes cobrindo todas as regras de cálculo e sincronização).',
  },
  {
    data: '2026-09-16',
    resumo: 'Autenticação & Sessão — Sessão Permanente para Usuário de TV Wallboard (`tv.fin`, `sessaoDiaria.ts`, `sessaoDiaria.test.ts`, `App.tsx`, `diretrizes.ts`): 1. Implementado suporte a sessão contínua/permanente para o usuário `tv.fin` (e identificadores ou perfis com cargo TV), impedindo o logout forçado diário à meia-noite e desconexões por inatividade; 2. Na inicialização do app (`App.tsx`), sessões ativas do usuário `tv.fin` não são descartadas por virada de dia e têm sua marcação diária renovada automaticamente; 3. Adicionado heartbeat periódico e listener de reconexão de rede (online/focus) que renova a sessão Supabase Auth e o dia local em background sem interrupção de exibição; 4. Direcionamento automático para o painel de parede Wallboard (`/financeiro/faturamento-gwjaco`) no login ou na abertura da raiz `/`; 5. Fallback tolerante a falhas na inicialização: preserva o perfil ativo em cache local caso ocorra oscilação momentânea de conexão ao consultar `core_perfis`; 6. Testes unitários cobrindo todos os cenários de sessão permanente com 100% de aprovação.',
  },
  {
    data: '2026-09-16',
    resumo: 'Suprimentos > Fretes — Renomeação de Página, Filtro Sem MIGO e Indicador de Frete por Item e Quantidade (`pages.ts`, `BahiaSulAnalyticsPanel.tsx`, `bahiasul.ts`, `bahiasul.test.ts`, `diretrizes.ts`): 1. Rótulo da página `/suprimentos/frete` atualizado no menu de navegação de "Estimador de Frete" para "Fretes"; 2. Implementado filtro rápido "Sem MIGO" no Acompanhamento de Entregas da Bahia Sul (`BahiaSulAnalyticsPanel.tsx`), ocultando entregas cujos itens já possuem registro de MIGO no SAP mesmo se a transportadora demorar para atualizar o status como entregue; 3. Coluna de Pedido SAP enriquecida com badges do status de MIGO (MIGO OK, MIGO Parcial ou Sem MIGO); 4. No modal de detalhamento do CTe, adicionado card de cálculo de indicadores de frete unitário ("Frete Médio / por Item" do PO e "Frete Unitário / por Quantidade"), além de colunas de rateio proporcional por linha e valor unitário estimado na tabela de itens do pedido; 5. Inclusão dos campos de status MIGO e indicadores de frete unitário na exportação Excel.',
  },
  {
    data: '2026-09-15',
    resumo: 'Suprimentos > Central de Compras — Seleção Múltipla de Transportadoras e Correção do Filtro de Promessa em "Sem MIGO" (`DiligenciamentoSemMigoTable.tsx`, `Compras.tsx`, `diligenciamento.ts`, `diligenciamento.test.ts`, `diretrizes.ts`): 1. Na aba "Sem MIGO" (Diligenciamento), substituído o filtro dropdown nativo de transportadora pelo componente padronizado `MultiSelectFilter`, permitindo seleção múltipla com checkboxes, busca integrada, chips de seleção ativa e opção para itens "Sem transportadora"; 2. Corrigida a divergência no filtro de Data de Promessa (`DateRangeFilter`): pedidos na visão "Sem MIGO" agora avaliam diretamente a data de previsão efetiva calculada (`item.previsaoEfetiva`, baseada na remessa do pedido + prazos de transporte por UF ou edição manual), eliminando o vazamento de itens com datas antigas da RM (`data_entrega_prevista`) cujas previsões de pedido caíam fora do período filtrado; 3. Atualizado o contador de itens localizados para refletir com exatidão a contagem visível após os filtros de diligenciamento; 4. Testes unitários com 100% de aprovação (36 testes cobrindo seleção múltipla de transportadoras, sentinelas e filtragem de promessa).',
  },
  {
    data: '2026-09-15',
    resumo: 'SSMA > Formulários RID — Opções de Resposta no Editor, Plano de Ação para Fechamento do RID, Menções de Usuários do SISTEN e Contagem de Dias em Aberto (`SsmaFormEditorModal.tsx`, `SsmaRidDetalhesModal.tsx`, `SsmaRidPlanosAcaoView.tsx`, `SsmaRidView.tsx`, `SsmaHub.tsx`, `ssmaApi.ts`, `types.ts`, migration `20260915180000_add_plano_acao_to_ssma_rid_desvios.sql`, `ssmaPlanoAcao.test.ts`, `diretrizes.ts`): 1. No editor de formulários do RID (`SsmaFormEditorModal.tsx`), adicionada a visualização e edição direta de Opções de Resposta dentro do card de cada pergunta da aba "Perguntas", com chips de opções, exclusão individual, adição rápida inline, ordenação e indicação de perguntas booleanas; 2. No modal de detalhes do RID (`SsmaRidDetalhesModal.tsx`), implementada seção interativa de Plano de Ação para Fechamento do RID, contendo seleção da área de destino da demanda, prazo limite, contagem dinâmica de dias em aberto (ou tempo até conclusão), campo de demanda com menção a usuários via `@` com busca exclusiva em usuários ativos do SISTEN (`core_perfis` e `localDb`), chips de responsáveis marcados, status do plano (Pendente, Em Andamento, Concluído), parecer de conclusão e ação direta para concluir e fechar o RID; 3. Nova janela/aba "Planos de Ação" no RID (`SsmaRidPlanosAcaoView.tsx`) apresentando cards e tabela consolidada de todas as demandas direcionadas, contagem de dias em aberto em cada ação, novo KPI de tempo médio em aberto das ações ativas, filtros por área, status e responsável marcado, atalho para abrir o RID e exportação CSV com dias em aberto; 4. Migração no Supabase adicionando coluna indexada `plano_acao jsonb` na tabela `ssma_rid_desvios` com RLS.',
  },
  {
    data: '2026-09-14',
    resumo: 'Suprimentos > Cadastros Gerais > Grupos Compradores — Classificação de Comprador Responsável por Setor Solicitante (`GestaoGrupoComprador.tsx`, `setorCompradorApi.ts`, `setorCompradorApi.test.ts`, `almoxarifadoRm.ts`, `AbrirRm.tsx`, `types.ts`, `sup_setor_compradores`, `diretrizes.ts`): 1. Substituída a regra que vinculava comprador responsável na abertura da RM por grupo de mercadorias SAP pela regra baseada no setor dono da solicitação (`solicitante_sector_id`), mapeando os 19 setores oficiais para os compradores responsáveis correspondentes (575 André, 314 Itana, 358 Isadora, 610 Giulia); 2. A tela de Gestão de Grupos Compradores foi reformulada com navegação em duas janelas: Janela Inicial "Compradores por Setor Solicitante" (ativa na abertura da RM, com edição rápida de comprador, KPIs de setores por comprador e busca textual) e Janela Secundária "Grupos de Mercadorias SAP" (mantida como consulta histórica/secundária, explicitamente sinalizada sem função na RM no momento); 3. Criação da tabela persistente `sup_setor_compradores` no Supabase com RLS e carga inicial dos 19 setores; 4. Atualização da abertura de RM (`AbrirRm.tsx` e `almoxarifadoRm.ts`) com prioridade para comprador por setor e fallback seguro; 5. Testes unitários com 100% de cobertura das resoluções por setor.',
  },
  {
    data: '2026-09-14',
  ...
  {
    data: '2026-09-21',
    resumo: 'Financeiro > Faturamento GW Jacobina — Simplificação de Status e Identificação nas Notas (`FinFaturamentoWallboard.tsx`, `FinFaturamentoDetalhesModal.tsx`): 1. Removida a opção e legenda de "Faturado, com restrição" da matriz e do Wallboard de parede; 2. Unificação visual de todos os tramos faturados sob a cor azul (`var(--series-1)`) e rótulo único "Faturado"; 3. Ajustado o modal de detalhes para exibir o badge consistente "Faturado"; 4. Adicionado o número sequencial do tramo na listagem de "Últimas notas emitidas" (ex: "Torre 9 - T4 - 3158").',
  },
  {
    data: '2026-09-14',
  ...
  {
    data: '2026-09-14',
    resumo: 'SSMA > Termo de Execução de Teste de Alcoolemia e Orientações (`SsmaAlcoolemiaView.tsx`, `exportSsmaAlcoolemiaPdf.ts`, `SsmaHub.tsx`, `App.tsx`, `Formularios.tsx`, migration `20260914140000_add_ssma_termo_alcoolemia_fields.sql`): 1. Implementado o formulário oficial FRM.SOC-0042 (Termo de Execução de Teste de Alcoolemia e Orientação Preventiva sobre Uso de Substâncias Psicoativas) no Módulo de SSMA; 2. Integração bidirecional com a Portaria: a lista diária carrega automaticamente os sorteados registrados pela portaria para aferição pelo SSMA; 3. Cadastro avulso de colaboradores e prestadores para testes não sorteados (Motivado por suspeita ou Pós-Acidente); 4. Modal do termo com autorização, seleção da razão do teste, registro do resultado (Negativo 0,00 mg/L, Positivo ou Recusa), local e examinador; 5. Em caso de resultado POSITIVO, exibição de alerta em destaque com emissão obrigatória do Termo Físico para assinatura imediata do colaborador; 6. Gerador de PDF oficial idêntico ao modelo impresso padrão TEN com as 6 orientações obrigatórias sobre substâncias psicoativas e campos de assinatura.',
  },
  {
    data: '2026-09-14',
    resumo: 'Portaria > Teste de Alcoolemia (`PortariaAlcoolemia.tsx`, `portariaApi.ts`, `exportPortariaPdf.ts`, `PortariaHub.tsx`, `App.tsx`, migration `20260914120000_create_port_alcoolemia_testes.sql`): 1. Implementado novo formulário oficial FRM.SGP-0015 (Teste de Alcoolemia) no módulo de Portaria; 2. Sorteio diário de colaboradores e prestadores de serviço com pesquisa integrada ao banco de RH Pessoas (`rh_pessoas`) e opção de digitação direta para terceiros / PJs; 3. Criação dinâmica do Livro Diário de Sorteados com geração sequencial de código `ALC-DDMMYY-INDICE` (Regra 2 do AGENTS.md); 4. Registro de horário, turno, código do etilômetro, vigilante operador, resultado (Negativo/Apto, Positivo, Recusa, Pendente), valor medido (mg/L), testemunha e observações; 5. Navegação diária por calendário com atalhos de Ontem/Hoje/Amanhã, filtros por turno e resultado, KPIs operacionais em tempo real e exportação do Livro Diário em PDF oficial com campo para assinaturas.',
  },
  {
    data: '2026-09-14',
    resumo: 'Financeiro > Faturamento GW Jacobina — Detalhes Interativos nos Gráficos (`FinFaturamentoWallboard.tsx`, `FinFaturamentoDetalhesModal.tsx`, `FinFaturamentoGwjaco.tsx`): 1. Implementada abertura de janela modal detalhada ao clicar em qualquer gráfico ou indicador visual do painel de parede (Wallboard); 2. Suporte a cliques no Gráfico de Ritmo (por mês e por semana), Gráfico por Tramo (T1 a T5), Matriz Torre × Tramo (células individuais e cabeçalho de torres), Lista de últimas notas e Cards de KPIs superiores; 3. Modal responsivo com mini-KPIs do recorte, abas de filtro por status (Todos, Faturados, Expedidos, Pendentes, Com Restrição), busca textual e botão de atalho para edição direta do lançamento.',
  },
  {
    data: '2026-09-12',
    resumo: 'Almoxarifado > Projetos > Visão Geral (`PainelVisaoGeral.tsx`, `PainelPosicao.tsx`): 1. A Matriz de Autonomia de Kits por Tramo (`MatrizAutonomiaKits.tsx`) foi levada para a Visão Geral (`/almoxarifado/projetos`), substituindo integralmente a antiga matriz de progresso; 2. O painel principal do módulo agora exibe a matriz operacional completa vinculada ao saldo real do almoxarifado e à composição da BOM por sub-kits (Fixadores, Plataforma, Escada | Avanti, Escada Acesso), com as cores oficiais de status 0 a 4, planejamento de semanas (W36, W37...) e modal de inspeção de componentes e itens gargalo; 3. A aba Posição & Autonomia passa a priorizar o Resumo Consolidado por Tramo (T1 a T5) e a gestão de inventário por peça.',
  },
  {
    data: '2026-09-12',
    resumo: 'Produção > Controle de Entrega — Gestão Visual em Cilindros de Torres, Tempo de Espera (Aging) e Painel de Decisão (`TorresEntregaVisual.tsx`, `ModalDetalheTramoEntrega.tsx`, `producaoEntrega.ts`, `producaoEntrega.test.ts`, `producaoApi.ts`, `ProducaoOperacao.tsx`, migration `20260912230000_create_prod_tramos_entrega.sql`): 1. Reformulação completa da visão de Controle de Entrega (`/producao/entrega`), inspirada na identidade visual física da fábrica (TEN Nordeste) com representação tridimensional das torres em cilindros segmentados (T5 Topo a T1 Base); 2. Mapeamento cromático oficial por processo: Expedido (Preto), Pátio (Verde), White (Amarelo - Montagem/Jato/Pintura), Internos (Laranja - última fase antes de acabamento superficial), Saw02 (Lilás - Marco Porta e SAW02), Saw03 (Azul) e Nav01 (Pêssego); 3. Monitoramento em tempo real do status de espera ("o que está aguardando") e tempo de permanência contínuo (aging em dias) com badges de criticidade visual (Normal, Alerta e Crítico > 4 dias); 4. Painel de Decisão Executivo com identificação de Quick Wins de conjunto ("Torres Quase Prontas 4/5" — prioridade focal da fábrica para despacho ao parque) e ranking ordenado de gargalos por posto; 5. Modal de gestão operacional ao clicar em qualquer tramo para editar etapa, tempo de espera e pendência com diagnóstico de impacto no conjunto da torre; 6. Legenda completa da fábrica com contadores dinâmicos por categoria e nota explicativa de Internos.',
  },
  {
    data: '2026-09-12',
    resumo: 'Almoxarifado > Projetos — Matriz de Autonomia de Kits por Tramo (`MatrizAutonomiaKits.tsx`, `ModalComposicaoBomKit.tsx`, `projetosKitsAutonomia.ts`, `projetosKitsAutonomia.test.ts`, `projetosApi.ts`, `useDadosProjetos.ts`, `PainelPosicao.tsx`, migration `20260912220000_create_proj_matriz_autonomia.sql`): 1. Implementada a visualização matricial dos kits por tramo para as torres idêntica à planilha operacional da fábrica, conectando o estoque físico real à BOM de engenharia; 2. Linhas agrupadas por Tramo (T5 a T1) e sub-kits internos (Fixadores, Plataforma, Escada | Avanti, Escada Acesso em T1) com as 4 cores de status padronizadas: 0 (Não atende / Vermelho), 1 (Estoque / Verde), 3 (OK Pátio / Azul - kits pagos da pré-montagem para a produção) e 4 (Expedido / Laranja - com número de série gravado); 3. Modal de inspeção da Composição do Kit na BOM ao clicar em qualquer linha ou célula, exibindo peças, consumo unitário, saldo no almoxarifado, cobertura e destaque de itens gargalo com exportação XLSX; 4. Persistência de planejamento de semanas por torre (W36, W37...) e edição rápida de células em `proj_matriz_autonomia_kits` e `proj_torres_planejamento`; 5. Alternância fluida no painel Posição & Autonomia entre a Matriz Operacional e o Resumo por Tramo.',
  },
  {
    data: '2026-09-12',
    resumo: 'Módulo Produção — Liberação de Qualidade da Fabricação de Torres, Blocos 1-2 (`pages.ts`, `moduleHomes.ts`, `Sidebar.tsx`, `App.tsx`, `producao.ts`, `producao.test.ts`, `producaoApi.ts`, `outbox.ts`, `ProducaoLancamentos.tsx`, `ProducaoConsulta.tsx`, `LancamentoModal.tsx`, `FichaVirolaModal.tsx`, `SeletorPessoaField.tsx`, `GaleriaFotosProducao.tsx`, `vite.config.ts`): 1. Novo grupo PRODUÇÃO no menu, migrando a lógica de liberação de qualidade do sistema legado NAV1 (Corte a Plasma, Chanfro, Calandra e Solda SAW nestes dois blocos; EVS/UT/Flange e etapas pós-UT chegam nos blocos seguintes); 2. Reaproveita a numeração torre/tramo já existente (`torreDaSerie`/`tramoDaSerie` em `projetos.ts` e as 345 unidades de `proj_tramos_gwjaco`) em vez de recriar a fórmula do NAV1 — a virola entra como o nível abaixo do tramo, semeada para o projeto inteiro (3.243 virolas, tabela `prod_virolas`); 3. Etapas viram DADO (`prod_etapas`, com `etapa_anterior_id` fazendo o encadeamento) em vez de código: acrescentar uma etapa nova é inserir uma linha; 4. Cabeçalho único `prod_lancamentos` para qualquer etapa (em vez de uma tabela por etapa como no NAV1), com RLS real "autor ou admin" (`form_pode_editar`), log de alteração campo a campo em `prod_alteracoes`, foto sempre comprimida e carimbada com data/hora (regra 1 do CLAUDE.md) e código `MODULO-DDMMYY-INDICE` (regra 2) gerado no banco; 5. Offline-first: fila de gravação (`src/lib/outbox.ts`, IndexedDB via `idb-keyval`) com idempotência por `client_id` (RPC `prod_registrar_lancamento` com `on conflict client_id do nothing`), reenviada ao voltar `online` e ao ganhar foco; 6. Service worker (`vite-plugin-pwa`, `registerType: autoUpdate`) faz precache do app shell para o SISTEN abrir mesmo sem sinal — navegação usa `NetworkFirst` (não o precache) para não colidir com a guarda de chunk stale do `ErrorBoundary`; 7. Ficha da Virola: linha do tempo da peça do corte à etapa atual, com fotos e histórico — rastreabilidade que existia como dado no NAV1 mas nunca era exibida.',
  },
  {
    data: '2026-09-11',
    resumo: 'Formulários > Almoxarifado > Recebimento e Contagem — Atualização Automática de Chegada Física no Almoxarifado (`RecebimentoAlmox.tsx`, `recebimentoAlmox.ts`, `recebimentoAlmoxApi.ts`, `localDb.ts`, `recebimentoAlmox.test.ts`): 1. No formulário F2 (Recebimento e Contagem / RCM), qualquer item marcado como conferido (ou entrega parcial) tem seu status de chegada física atualizado automaticamente em `almoxarifado_chegadas` ao salvar a conferência (novo registro ou edição); 2. A resolução do identificador `ri_po` é executada automaticamente cruzando o pedido e código de material com o cache SAP (`localDb.getEnrichedSAPRequisicoes()`) ou recuperando de `linhaRef`, gravando a data da conferência e usuário logado; 3. O detalhamento da conferência passa a exibir o selo "chegada almox" nos itens cuja chegada física está registrada; 4. Adicionados testes unitários em `recebimentoAlmox.test.ts` validando a extração dos `ri_po` para itens conferidos e descarte de avarias/não conferidos.',
  },
  {
    data: '2026-09-11',
  ...
  {
    data: '2026-09-10',
    resumo: 'Cotações > Importação de Propostas — Conversão Automática ao Carregar, Botão "Ver md" e Extração com IA (`pdfNativeExtract.ts`, `pdfNativeExtract.test.ts`, `ImportarPropostasPanel.tsx`, `ItemFilaRow.tsx`): 1. Implementada extração nativa de texto de PDFs no navegador via `pdfjs-dist` (custo R$ 0,00 e conversão instantânea em milissegundos para PDFs gerados por ERPs/sistemas), com fallback transparente para OCR com IA caso o arquivo seja escaneado ou foto; 2. Conversão automática ao carregar: ao soltar ou selecionar propostas, a conversão para Markdown inicia imediatamente em segundo plano; 3. Botão "Ver md": para arquivos já convertidos (ou recuperados do cache/duplicatas), a linha exibe o botão direto "Ver md" para inspecionar o Markdown e o documento original lado a lado; 4. Ação simplificada: removido o passo intermediário de conversão manual, mantendo na interface apenas o botão principal "Extrair cotação com IA"; 5. Correção de evento de clique em `handleExtrairCotacao` garantindo validação segura com `Array.isArray`.',
  },
  {
    data: '2026-09-10',
    resumo: 'Nova Solicitação — Tipo de Compra, Alinhamento com Almoxarifado e Crítica para Item Genérico (`NewRequest.tsx`, `solicitacoes.ts`, `solicitacoes.test.ts`): 1. "Direta" é agora a primeira opção de Tipo de Compra (`Direta`, `Estoque`, `Serviço`) e o formulário inicia sem nenhuma selecionada (`null`), exigindo escolha consciente do usuário antes do envio; 2. Ao selecionar "Estoque", exibe aviso e seleção obrigatória ("Avisou ao almoxarifado sobre a compra? [Sim, avisei / Não avisei]"), exceto quando o usuário logado pertence ao setor de Almoxarifado (`sector_id = 2`), registrando a confirmação na justificativa; 3. Quando o usuário marca "Item Genérico" em compras com tipo "Estoque", é exibida uma crítica visual em destaque orientando que itens de estoque necessitam de cadastro SAP para controle físico e inventário, com botão de atalho para "Cadastrar item no SAP".',
  },
  {
    data: '2026-09-10',
    resumo: 'Contratos > Demandas — Correção de RLS para Movimentação de Cards no Quadro Kanban (`TabDemandas.tsx`, `DemandaDetailModal.tsx`, `localDb.ts`, migration `20260910170000`): 1. Usuários do setor Jurídico ou com acesso liberado a Contratos (`page_access.sup_contratos`) tinham a movimentação de cards bloqueada por RLS no Supabase ao arrastar chamados criados por outros usuários, causando reversão imediata do card para "Aberto"; 2. Criada função SQL `pode_gerir_contratos()` e atualizada a policy `requests_update` em `core_solicitacoes` para autorizar a edição por membros do setor Jurídico (`sector_id = 17` ou nome correspondente), usuários com `sup_contratos` e membros do setor de destino do chamado (`target_sector_id`); 3. Em `localDb.ts`, `transitionRequestStatus` e `updateRequestStatus` agora recebem `actorId` com fallback seguro e associam o atendente automaticamente caso esteja desatribuído; 4. Adicionado feedback visual via Toast em `TabDemandas.tsx` e validação de sucesso no modal de detalhes.',
  },
  {
    data: '2026-09-09',
    resumo: 'RH > ASE - Hora Extra — Resolução Definitiva de Protocolos Duplicados e Concorrência (`rhApi.ts`, `RhAseHoraExtra.tsx`, trigger `trg_rh_ase_garantir_protocolo_unico`, `diretrizes.ts`): 1. Implementada trigger `BEFORE INSERT OR UPDATE` no Supabase (`trg_rh_ase_garantir_protocolo_unico`) que intercepta tentativas de salvar com `numero_protocolo` já existente e calcula automaticamente o próximo sequencial disponível (`-01`, `-02`), protegendo contra violações de chave única independentemente de versão em cache do navegador; 2. Em `salvarSolicitacaoASE`, adicionada leitura de retorno do protocolo persistido via `.select(\'numero_protocolo\')`, e caso haja conflito e os dados de setor/data não venham no patch, busca os dados da própria solicitação no banco para recalcular o protocolo com a sigla correta do setor; 3. Em `RhAseHoraExtra.tsx`, desacoplada a sincronização assíncrona do protocolo de modo a não atribuir prematuramente um protocolo com colisão no estado local durante seleção de cabeçalho; 4. Adicionadas dependências `markmap-view` e `markmap-common` em `package.json` para desbloquear o build de produção no CI/CD.',
  },
  {
    data: '2026-09-09',
    resumo: 'Solicitações > Redesenho Completo da Central de Solicitações (`SolicitacoesCentral.tsx`, `solicitacoesTabelaHelpers.ts`, `solicitacoesTabelaHelpers.test.ts`, `diretrizes.ts`): 1. Nova barra superior com pílulas de tipo (Todas, Compras, Cadastros SAP, Chamados) e alternador integrado de modos de exibição (Lista, Quadro Kanban e Calendário de prazos); 2. Barra unificada de busca e filtros refinados com rótulos superiores (Status com padrão "Abertas", Criticidade, Meu setor, Data e botão de limpeza); 3. Tabela moderna com 10 colunas: número em destaque (#5001001), tipo com ícone temático e tag em caixa alta, título e justificativa em duas linhas, solicitante com avatar de iniciais, setor, data de abertura com tempo relativo, prazo com aviso de vencimento em vermelho, pílula de criticidade e status, e menu de ações rápidas.',
  },
  {
    data: '2026-09-10',
    resumo: 'Formulários > Almoxarifado > Recebimento — Paleta das Tags/Chips (`RecebimentoAlmox.tsx`): as tags de status do módulo (OK, DIVERG., NCR, parcial, severidade) usavam a rampa `--abc-a/b/c`, que em `tokens.css` é **toda azul** (é a rampa ordinal ABC de gráfico) — daí "NCR" e "DIVERG." saírem lavanda/rosa lavado. Migradas para a escala reservada `--status-*` (`--status-good` verde / `--status-serious` laranja / `--status-critical` vermelho) com fundo `color-mix(... N%, transparent)`, mesmo padrão de `DivergenciaPmmPanel`/`CompraEvitavelPanel`. `StatusChip` reescrito (tons `ok`/`atencao`/`alerta`/`neutro`); badge "NCR" agora vermelho sólido com texto branco + ícone; item divergente = vermelho, parcial = laranja, conferido ok = verde.',
  },
  {
    data: '2026-09-10',
    resumo: 'Formulários > Almoxarifado > Recebimento e Contagem — Check "Parcial" por Item / Entrega Parcial do PO (`RecebimentoAlmox.tsx`, `recebimentoAlmox.ts`, migration `20260910140000`): 1. Nova coluna `alm_receb_conferencia_itens.parcial` e check "parcial" por linha — reaproveita a lógica de entrega parcial da Central de Compras (coluna `qtd_fornecida` da ZL0132): quando o lote que chegou ainda é MENOS que o pendente mas está certo (o resto vem depois), marca-se "parcial" e `classificarDivergencia` deixa de acusar `falta` — **não abre NC**; excedente e avaria continuam flagrando; `resumoConferencia` passa a contar `parciais`; 2. Ao "Puxar itens" de um PO que já tem entrega parcial, alerta em toast + selo por item ("Entrega parcial no PO — já recebido X de Y · pendente Z"); o `qtdRecebida` default já vem como `qtd_pedido − qtd_fornecida` (pendente descontado); 3. Visual: linha parcial ganha borda/realce âmbar + chip "parcial", distinta do verde (conferido ok) e do vermelho (divergência); contadores do formulário e do detalhamento mostram "N ok · N parcial · N diverg.".',
  },
  {
    data: '2026-09-10',
    resumo: 'UI > Visualizador de Imagens Padrão + Melhorias Visuais no Recebimento (`src/components/ui/Lightbox.tsx`, `Attachments.tsx`, `views/almoxarifado/RecebimentoAlmox.tsx`): 1. Novo componente `Lightbox` + hook `useLightbox()` — as imagens do SISTEN abrem **dentro do app** (overlay com Esc/setas/contador), nunca mais em outra aba; `AttachmentGallery` (Solicitações, Cadastros SAP, Helpdesk, Rastreio) e o módulo de Recebimento passam a usá-lo (o `<a target="_blank">` das miniaturas virou botão que abre o visualizador); 2. No detalhamento e nos formulários de Recebimento, item com divergência ganha **borda/tarja vermelha + tag "NCR"/tipo**, e item conferido sem divergência ganha **borda/realce verde + selo "conferido"**; chips de status coloridos padronizados (`StatusChip`: ok/alerta/info/neutro) para status da NCR, severidade, resumo da conferência; 3. Fotos nas linhas de item aparecem como **ícone pequeno com contagem** que abre o visualizador na mesma janela.',
  },
  {
    data: '2026-09-10',
    resumo: 'Formulários > Almoxarifado > Recebimento — Detalhamento, Multi-PO, Carimbo de Foto e NCR Editável (`RecebimentoAlmox.tsx`, `carimboFoto.ts`, `recebimentoAlmoxApi.ts`, migrations `20260910120000` e `20260910130000`): 1. Todo cartão (ficha cega, conferência, NCR) abre um modal de **detalhamento** read-only com o preenchimento e, no fim da janela, o **log de alterações** (o botão "Histórico" foi absorvido pelo detalhe); 2. A conferência aceita **vários POs** — campo "Adicionar PO" acumula pedidos, cada item guarda seu `nro_pedido` (nova coluna em `alm_receb_conferencia_itens`) e `alm_receb_conferencias.pedidos text[]` lista todos (principal = 1º, via helper `alm_receb_pedidos_ordenados` que deduplica preservando ordem); 3. Foto ligada a cada item (galeria por linha) com campo separado para a **foto geral** do recebimento; 4. Toda foto do módulo passa por `prepararFotoCarimbada` (`carimboFoto.ts`): canvas único que redimensiona para 1600px e **queima `DD/MM/AAAA HH:MM`** numa tarja no canto antes de codificar JPEG (PDF/HEIC caem no `prepareAttachment` sem carimbo); 5. NCR **editável** (`ModalNc` + RPC `alm_receb_editar_nc`, security definer, sem trava de autor pois a tratativa é de terceiros): status/severidade/tipo/responsável/descrição/resolução, **registrar ação** (observação + foto → entrada em `alm_receb_nc.acoes[]` com autor e timestamp) e fotos gerais; toda mudança loga em `alm_receb_alteracoes` (entidade `nc`), inclusive o atalho de status na lista.',
  },
  {
    data: '2026-09-09',
    resumo: 'Formulários > Almoxarifado — Recebimento de Material (novo módulo: `src/views/almoxarifado/RecebimentoAlmox.tsx`, `src/lib/recebimentoAlmox.ts`, `recebimentoAlmoxApi.ts`, migrations `20260909170000/170100/170200_*alm_recebimento*.sql`): 1. **F1 Ficha cega de volumes (RCV)** — contagem cega de volumes na doca da transportadora, `divergencia` automática quando contagem ≠ declarado ou avaria aparente; campo de transportadora com autocomplete (itens sem MIGO de `sup_diligenciamento_itens` + cadastro `sup_transportadoras` + cargas já lançadas, aceita digitar e memoriza), campos de texto sempre em MAIÚSCULAS, PO só dígitos, fotos opcionais (até 24 — migration `20260909170400` removeu a obrigatoriedade da RPC); 2. **F2 Recebimento e contagem (RCM)** — puxa a lista de itens do PO com dupla contingência (cache SAP `localDb.getEnrichedSAPRequisicoes()` → RPC `alm_receb_po_linhas` sobre `sap_zl0132_po` → digitação manual), valida quantidade item a item contra `qtd_pedido` − `qtd_ja_fornecida` com tolerância de ponto flutuante, check/foto/observação por item; 3. Divergência (falta, excedente, avaria, material trocado, fora do pedido) abre automaticamente uma **Não Conformidade de Recebimento (NCR)** consolidada — uma por conferência — em `alm_receb_nc`, com resumo dos itens e evidências; 4. Serve item de projeto (código 100000…) e de consumo no mesmo formulário; conferência de item de projeto exibe CTA "Registrar entrada em Projetos" que marca `encaminhado_projetos` e navega para `/almoxarifado/projetos/recebimento`; 5. Código dos registros gerado na RPC (`alm_receb_proximo_codigo`), padrão `MODULO-DDMMYY-INDICE` com índice reiniciando por dia e à prova de colisão entre aparelhos offline (`unique(codigo)` como rede final); 6. Edição pelo autor: só quem abriu o registro (ou admin) edita — RLS por tabela via `form_pode_editar` (migration `20260909170500`), espelhada no cliente por `podeEditarFormulario`; "Editar" reabre o modal preenchido e grava por `alm_receb_editar_carga`/`alm_receb_editar_conferencia` (recalculam divergência/status/contadores e abrem NCR se a conferência passou a divergir); toda edição é registrada em `alm_receb_alteracoes` (campo/de/para/quem/quando), exibida em "Histórico" no cartão; **excluir é lógico** (`excluido_em`) — sai da tela, permanece no banco; 7. RPCs transacionais (`alm_receb_registrar_carga`, `alm_receb_registrar_conferencia`) gravam cabeçalho + itens + NCR numa única transação; bucket privado `alm-recebimento` (espelha `proj-evidencias`), foto sempre comprimida (regra 1 do CLAUDE.md); a tela `/formularios/almoxarifado` é um hub de **cards** no mesmo padrão da Portaria (`PortariaHub`) — cada card abre uma vista dedicada e o formulário de lançamento abre como modal —, card do hub de Formulários ativado em `Formularios.tsx`, rota em `App.tsx`; migrations aplicadas (`20260909170000`/`170100`/`170200`/`170300` — esta revoga `execute` do papel `anon`, pois `revoke from public` não basta no Supabase).',
  },
  {
    data: '2026-09-09',
    resumo: 'Contratos & Demandas > Correção de Erro de Hook em TabDemandas (`vite.config.ts`, `TabDemandas.tsx`, `ViewQuadro.tsx`, `diretrizes.ts`): 1. Correção do erro de execução "Invalid hook call. Cannot read properties of null (reading \'useMemo\') at TabDemandas:484" originado por instâncias duplicadas de React geradas pelo pre-bundler do Vite entre módulos da aplicação e dependências CJS/ESM do `@dnd-kit/core`; 2. Configurada desduplicação mandatória de instâncias de `react` e `react-dom` em `resolve.dedupe` e otimização explícita de `@dnd-kit/core`, `@dnd-kit/utilities` e `@dnd-kit/accessibility` no `vite.config.ts`; 3. Extração de configuração estável de sensor (`SENSOR_POINTER_OPTIONS`) evitando recriações desnecessárias.',
  },
  {
    data: '2026-09-09',
    resumo: 'Suprimentos > Cadastros SAP — Tratamento de Solicitações Canceladas, Correção de SLA e Exclusão Definitiva (`CadastrosSap.tsx`, `localDb.ts`, `cadastrosSap.test.ts`, `diretrizes.ts`): 1. Na aba "Fila Pendente" de Cadastros SAP, excluídas da visualização solicitações canceladas, rejeitadas, resolvidas e fechadas, mantendo em fila estritamente chamados abertos e pendentes de atendimento; 2. Correção no cálculo de SLA para solicitações canceladas e rejeitadas, substituindo a contagem incorreta de atraso ("Atrasado XXh") pelo badge neutro "Cancelada" / "Rejeitada"; 3. Ajuste nos botões de ação ("Ver" para chamados encerrados em vez de "Atender ->") e ocultação dos formulários de triagem/atendimento em solicitações canceladas; 4. Implementação de exclusão definitiva (`localDb.deleteRequest`) com confirmação em modal (`ConfirmDialog`), removendo registros no Supabase e no cache local; 5. Purge automático de solicitações de teste legadas e canceladas.',
  },
  {
    data: '2026-09-10',
    resumo: 'RH > ASE - Hora Extra — Padronização de Protocolos com Setor e Índice Obrigatório (`rhApi.ts`, `RhAseHoraExtra.tsx`, `rhApi.test.ts`, `20260910020000_ase_migrar_codigos_indice.sql`): 1. Padronização oficial do protocolo no formato `ASE-DDMMAA-SETOR-INDICE` (Opção B, ex.: `ASE-090926-PROD-01`, `ASE-090926-PROD-02`), garantindo que todos os registros possuam índice sequencial de no mínimo 2 dígitos desde a primeira ocorrência do dia; 2. Migração completa e segura de 100% dos registros passados vigentes no Supabase para o padrão com índice (`-01`, `-02`), eliminando códigos estáticos e inconsistências no histórico; 3. Isolamento dos registros excluídos com prefixo `ASE-DEL-...` para liberar a restrição de unicidade (`rh_ase_solicitacoes_numero_protocolo_key`) contra colisões com novas solicitações; 4. Ativação de trigger de integridade no PostgreSQL para resolução atômica de concorrência e incremento sequencial automático em caso de tentativas simultâneas.',
  },
  {
    data: '2026-09-09',
    resumo: 'Suprimentos > Rastreio de Compras — Tag e Observação de Item Genérico na Tabela e Detalhes (`rastreio.ts`, `RastreioCompras.tsx`, `RastreioTable.tsx`, `RastreioDetailModal.tsx`, `rastreio.test.ts`, `diretrizes.ts`): 1. Na listagem de Rastreio de Compras (tabela desktop e cards mobile), itens genéricos vinculados a solicitações do SISTEN com `is_generic: true`, com observação prefixada com "ITEM GENÉRICO:" ou identificados com tags `[GENÉRICO]`/`[IG]` passam a exibir a tag destacada "Item Genérico" junto ao código do material; 2. Exibição da especificação/observação técnica do solicitante em itálico logo abaixo da descrição breve do item; 3. No modal de detalhes do rastreio (`RastreioDetailModal.tsx`), adicionada badge de Item Genérico no cabeçalho e campo dedicado com a especificação original do solicitante; 4. Suporte na busca textual do rastreio para filtrar por itens genéricos ("generico" / "genérico") e termos contidos na observação técnica.',
  },
  {
    data: '2026-09-09',
    resumo: 'Suprimentos > Cadastros SAP — Campo de Observações/Andamentos sem Pausar SLA e Conversa da Solicitação (`CadastrosSap.tsx`, `localDb.ts`, `cadastrosSap.test.ts`): 1. Adicionado campo dedicado para "Observações / Andamento" no painel de atendimento do chamado em `CadastrosSap.tsx`, permitindo ao atendente registrar andamentos (ex.: "Chamado aberto no Astrein nº 507203") diretamente na conversa da solicitação sem alterar o status e sem pausar a contagem de SLA; 2. Diferenciação visual clara entre "Observações / Andamento" (SLA ativo) e "Solicitar Esclarecimento" (que pausa o SLA ao transicionar para "Aguardando Solicitante"); 3. Exibição da linha do tempo da conversa e andamentos da solicitação diretamente no painel lateral de detalhes; 4. Reativação automática do SLA para "Em Atendimento" caso o solicitante responda a um chamado de cadastro SAP em aguardo; 5. Testes unitários cobrindo o fluxo de observações e pausamento/retomada de SLA.',
  },
  {
    data: '2026-09-09',
    resumo: 'Suprimentos > Cadastros SAP — Permissão RLS no Supabase e Resiliência na Gestão de Solicitações (`CadastrosSap.tsx`, `localDb.ts`, `20260909120000_core_solicitacoes_cadastro_sap_rls.sql`): 1. Correção de política de RLS em `core_solicitacoes`: criada função segura `pode_gerir_cadastro_sap()` e `requests_pode_editar()` garantindo que analistas e responsáveis por cadastros SAP (com permissão `sup_cadastros_sap` em `page_access` ou `aprovador_cadastro_sap`) bem como atendentes atribuídos (`atendente_id`) e compradores (`comprador_id`) possam assumir, pausar SLA ("Solicitar Esclarecimento") e resolver solicitações sem violação de segurança do PostgreSQL; 2. Ajustado `assignAtendente` em `localDb.ts` para retornar `Promise<boolean>` com rollback do cache local em caso de falha de persistência, e adicionada validação com mensagem de erro amigável em `CadastrosSap.tsx`.',
  },
  {
    data: '2026-09-09',
  ...
  {
    data: '2026-09-09',
    resumo: 'Almoxarifado > Abrir RM / Numeração de Itens no Padrão SAP (* 10) (`almoxarifadoRm.ts`, `AbrirRm.tsx`, `almoxarifadoRm.test.ts`, `diretrizes.ts`): 1. Padronizado o número do item na exportação da planilha de abertura de RM para seguir a convenção padrão do SAP (incremento de 10: item 1 = 10, item 2 = 20, item 3 = 30...); 2. Atualizado o texto de cabeçalho da RM (`Texto Cabeçalho (Justificativa)`) para referenciar as observações com o respectivo item SAP (ex: `-Item 10 [OBS]; -Item 20 [OBS];`); 3. Atualizada a exibição da coluna "Item" na tabela expandida de itens de cada solicitação e no modal de pré-visualização para refletir consistentemente os números 10, 20, 30...; 4. Suíte de testes unitários atualizada e validada.',
  },
  {
    data: '2026-09-09',
    resumo: 'Suprimentos > Frete Bahia Sul / Inclusão de GRIS (0,5%) e Correção de Divergência Contratual (`bahiasul.ts`, `FreteEstimator.tsx`, `BahiaSulAnalyticsPanel.tsx`, `localDb.ts`, `types.ts`, `bahiasul.test.ts`): 1. Incluído o componente contratual de GRIS (Gerenciamento de Risco, 0,5% sobre o valor da mercadoria conforme seção Generalidades da Bahia Sul) no motor de cálculo `calcularFreteContratual`, no simulador e na memória discriminada de custos; 2. Corrigida a divergência em CTes onde o frete cobrado era erroneamente apontado com sobrepreço pela falta do GRIS (ex: CTe nº 00033095 de R$ 195,87 que apresentava +R$ 27,38 de falso sobrepreço e agora calcula exatos R$ 195,87 ficando 100% Conforme); 3. Aplicado arredondamento monetário de duas casas decimais por componente antes da composição da base de cálculo do ICMS por dentro (padrão fiscal SEFAZ); 4. Adicionado o card de GRIS na memória decomposta do modal de detalhes do CTe, no resumo copiado e na exportação Excel.',
  },
  {
    data: '2026-09-05',
        ...
        secoes: [
          {
            ...
            itens: [
              ...
              'Fallback ME2L dentro da própria `vw_sap_requisicoes_enriquecidas` (migrations `me2l_fallback_sem_po_contrato` de 2026-09-12 e `fix_me2l_fallback_material_match` de 2026-09-14): a extração ZL0132 do SAP tem uma lacuna real — ~67% dos pedidos que são call-off de contrato não aparecem nela. Quando a ZL0132 não acha nenhum PO para a RI (p.doc_compra IS NULL), a view faz fallback com `sap_me2l_pedido` casando obrigatoriamente por "Requisição de compra" E pelo código do "Material" (para materiais) ou por requisição (para serviços sem material, RMs 17), evitando que pedidos parciais de um item contaminem outros itens em aberto da mesma RM (como ocorria na RM 1100334325). Preenche `documento_compra`/`contrato_po`/`status_requisicao`/`origem_po`. Column `origem_po` (\'zl0132\' | \'me2l\' | null) existe só para auditoria/debug, não é consumida no front hoje.',
        ...
        secoes: [
          ...
          },
          {
            ...
            itens: [
              ...
              'Fluxo: aberto → em_atendimento (ao "Assumir") → aguardando_solicitante (pede esclarecimento, pausa SLA) → resolvido (nota + campo dedicado para digitar o "Código SAP do Fornecedor" ou "Cód. Material SAP", persistindo em `codigo_fornecedor_sap` e `codigo_sap_gerado`) → fechado.',
              'Atendente que assumiu, bem como perfis de coordenação/administração (admin, coordenador_suprimentos), acessam os formulários de esclarecimento e fechamento da demanda, além de permitir atualização pontual do código SAP pós-fechamento.'
            ]
          },
          {
            titulo: 'Tabelas do banco (Supabase)',
    ...
    paginas: [
      ...
      },
      {
        ...
        secoes: [
          ...
          {
            titulo: 'Validação de briefing de segurança (validade: 6 meses)',
            itens: [
              'Validade estrita de 6 meses (180 dias) para treinamentos de integração/briefing.',
              ...
              'Ativação automática e obrigatória de "Fará Briefing" se qualquer visitante estiver vencido (>6 meses) ou sem histórico.',
        ...
        secoes: [
          {
            ...
            itens: [
              'Código identificador padronizado no formato `ASE-DDMMAA-SETOR-INDICE` (ex.: `ASE-090926-PROD-01`) com índice sequencial obrigatório de no mínimo dois dígitos por setor e data, gerado por `gerarProtocoloAse` em `src/lib/rhApi.ts`.'
            ]
          },
          {
